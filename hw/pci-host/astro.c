/*
 * HP-PARISC Astro/Pluto/Ike/REO system bus adapter (SBA)
 * with Elroy PCI bus (LBA) adapter emulation
 * Found in C3000 and similar machines
 *
 * (C) 2023-2026 by Helge Deller <deller@gmx.de>
 *
 * This work is licensed under the GNU GPL license version 2 or later.
 *
 * Chip documentation is available at:
 * https://parisc.docs.kernel.org/en/latest/technical_documentation.html
 *
 * User-added devices attach to the first Elroy PCI bus; attachment to the
 * other Elroy buses is not implemented.
 */

#define TYPE_ASTRO_IOMMU_MEMORY_REGION "astro-iommu-memory-region"

#include "qemu/osdep.h"
#include "qemu/module.h"
#include "qemu/units.h"
#include "qapi/error.h"
#include "hw/core/irq.h"
#include "hw/pci/pci_device.h"
#include "hw/pci/pci_bus.h"
#include "hw/core/qdev-properties.h"
#include "hw/pci-host/astro.h"
#include "hw/pci-host/hp-io-sapic.h"
#include "hw/pci-host/hp-lba-config.h"
#include "hw/pci-host/hp-sba-iommu.h"
#include "hw/pci-host/hp-sba-routing.h"
#include "hw/hppa/hppa_hardware.h"
#include "migration/vmstate.h"
#include "target/hppa/cpu.h"
#include "trace.h"
#include "qom/object.h"
#include "exec/target_page.h"

static bool astro_direct_route_lookup(const HPSBARoutingVariant *variant,
                                      uint64_t route,
                                      unsigned int *root_index)
{
    (void)variant;
    /* Astro uses low-32-bit, route-and-three decode. */
    *root_index = (uint32_t)route & (ELROY_NUM - 1);
    return true;
}

static uint64_t astro_extend_address(void *opaque, uint64_t address)
{
    (void)opaque;
    return address | MAKE_64BIT_MASK(32, 32);
}

static void astro_deliver_interrupt(void *opaque, uint64_t address,
                                    uint32_t data)
{
    AstroState *s = opaque;

    stl_be_phys(&address_space_memory,
                hp_sba_routing_extend_address(s->variant, s, address), data);
}

static const HPSBARootSpec astro_roots[ELROY_NUM] = {
    { .hpa_offset = 0x30000, .bus_num = 0, .user_attachable = true },
    { .hpa_offset = 0x32000, .bus_num = 1 },
    { .hpa_offset = 0x38000, .bus_num = 2 },
    { .hpa_offset = 0x3c000, .bus_num = 3 },
};

static const HPSBARopeMap astro_rope_map[ELROY_NUM] = {
    { .rope = 0, .root = 0 },
    { .rope = 1, .root = 1 },
    { .rope = 4, .root = 2 },
    { .rope = 6, .root = 3 },
};

static const HPSBARoutingVariant astro_routing_variant = {
    .roots = astro_roots,
    .root_count = ARRAY_SIZE(astro_roots),
    .rope_map = astro_rope_map,
    .rope_map_count = ARRAY_SIZE(astro_rope_map),
    .physical_rope_count = ROPES_PER_IOC,
    .direct_range_count = LMMIO_DIRECT_RANGES,
    .sba_reg_endianness = DEVICE_LITTLE_ENDIAN,
    .lba_reg_endianness = DEVICE_LITTLE_ENDIAN,
    .config_endianness = DEVICE_LITTLE_ENDIAN,
    .direct_route_lookup = astro_direct_route_lookup,
    .extend_address = astro_extend_address,
    .deliver_interrupt = astro_deliver_interrupt,
};

/*
 * Helper functions
 */

static uint64_t mask_32bit_val(enum device_endian endianness, hwaddr addr,
                               unsigned int size, uint64_t val)
{
    uint64_t result;

    assert(hp_sba_reg64_read(endianness, val, addr, size, &result));
    return result;
}

static void put_val_in_int64(enum device_endian endianness, uint64_t *p,
                             hwaddr addr, unsigned int size, uint64_t val)
{
    assert(hp_sba_reg64_write(endianness, p, addr, size, val));
}

static void put_val_in_array(enum device_endian endianness, uint64_t *array,
                             hwaddr start_addr, hwaddr addr,
                             unsigned int size, uint64_t val)
{
    int index;

    index = (addr - start_addr) / 8;
    put_val_in_int64(endianness, &array[index], addr, size, val);
}


/*
 * The Elroy PCI host bridge. We have at least 4 of those under Astro.
 */

static MemTxResult elroy_chip_read_with_attrs(void *opaque, hwaddr addr,
                                             uint64_t *data, unsigned size,
                                             MemTxAttrs attrs)
{
    MemTxResult ret = MEMTX_OK;
    ElroyState *s = opaque;
    uint64_t val = -1;
    int index;

    switch ((addr >> 3) << 3) {
    case 0x0008:
        val = 0x6000005; /* func_class */
        break;
    case 0x0058:
        /*
         * Scratch register, but firmware initializes it with the
         * PCI BUS number and Linux/HP-UX uses it then.
         */
        val = s->pci_bus_num;
        /* Upper byte holds the end of this bus number */
        val |= s->pci_bus_num << 8;
        break;
    case 0x0080:
        val = s->arb_mask; /* set ARB mask */
        break;
    case 0x0108:
        val = s->status_control;
        break;
    case 0x200 ... 0x250 - 1: /* LMMIO, GMMIO, WLMMIO, WGMMIO, ... */
        index = (addr - 0x200) / 8;
        val = s->mmio_base[index];
        break;
    case 0x0680:
        val = s->error_config;
        break;
    case 0x0688:
        val = 0;                /* ERROR_STATUS */
        break;
    case 0x0800:                /* IOSAPIC_REG_SELECT */
        val = hp_io_sapic_select_read(&hp_io_sapic_astro_policy,
                                      s->iosapic_reg_select);
        break;
    case 0x0810:                /* IOSAPIC_REG_WINDOW */
        if (!hp_io_sapic_window_read(&hp_io_sapic_astro_policy,
                                     s->iosapic_reg_select,
                                     s->iosapic_reg,
                                     ARRAY_SIZE(s->iosapic_reg), &val)) {
            goto check_hf;
        }
        trace_iosapic_reg_read(s->iosapic_reg_select, size, val);
        break;
    default:
    check_hf:
        if (s->status_control & HF_ENABLE) {
            val = 0;
            ret = MEMTX_DECODE_ERROR;
        } else {
            /* return -1ULL if HardFail is disabled */
            val = ~0;
            ret = MEMTX_OK;
        }
    }
    trace_elroy_read(addr, size, val);

    /* for 32-bit accesses mask return value */
    val = mask_32bit_val(s->astro->variant->lba_reg_endianness,
                         addr, size, val);

    trace_astro_chip_read(addr, size, val);
    *data = val;
    return ret;
}


static MemTxResult elroy_chip_write_with_attrs(void *opaque, hwaddr addr,
                                              uint64_t val, unsigned size,
                                              MemTxAttrs attrs)
{
    ElroyState *s = opaque;

    trace_elroy_write(addr, size, val);

    switch ((addr >> 3) << 3) {
    case 0x000: /* PCI_ID & PCI_COMMAND_STATUS_REG */
        break;
    case 0x080:
        put_val_in_int64(s->astro->variant->lba_reg_endianness,
                         &s->arb_mask, addr, size, val);
        break;
    case 0x0108:
        put_val_in_int64(s->astro->variant->lba_reg_endianness,
                         &s->status_control, addr, size, val);
        break;
    case 0x200 ... 0x250 - 1:   /* LMMIO, GMMIO, WLMMIO, WGMMIO, ... */
        put_val_in_array(s->astro->variant->lba_reg_endianness,
                         s->mmio_base, 0x200, addr, size, val);
        break;
    case 0x300: /* ibase */
    case 0x308: /* imask */
        break;
    case 0x0680:
        put_val_in_int64(s->astro->variant->lba_reg_endianness,
                         &s->error_config, addr, size, val);
        break;
    case 0x0800:                /* IOSAPIC_REG_SELECT */
        hp_io_sapic_select_write(&hp_io_sapic_astro_policy,
                                 &s->iosapic_reg_select, val);
        break;
    case 0x0810:                /* IOSAPIC_REG_WINDOW */
        trace_iosapic_reg_write(s->iosapic_reg_select, size, val);
        if (!hp_io_sapic_window_write(&hp_io_sapic_astro_policy,
                                      s->iosapic_reg_select,
                                      s->iosapic_reg,
                                      ARRAY_SIZE(s->iosapic_reg), val)) {
            goto check_hf;
        }
        break;
    case 0x0840:                /* IOSAPIC_REG_EOI */
        val = le64_to_cpu(val);
        hp_io_sapic_eoi(&hp_io_sapic_astro_policy,
                        s->iosapic_reg, ARRAY_SIZE(s->iosapic_reg),
                        &s->ilr, val, 0, NULL, NULL);
        break;
    default:
    check_hf:
        if (s->status_control & HF_ENABLE) {
            return MEMTX_DECODE_ERROR;
        }
    }
    return MEMTX_OK;
}

static const MemoryRegionOps elroy_chip_ops = {
    .read_with_attrs = elroy_chip_read_with_attrs,
    .write_with_attrs = elroy_chip_write_with_attrs,
    .endianness = DEVICE_LITTLE_ENDIAN,
    .valid = {
        .min_access_size = 4,
        .max_access_size = 8,
    },
    .impl = {
        .min_access_size = 4,
        .max_access_size = 8,
    },
};


/* Unlike pci_config_data_le_ops, no check of high bit set in config_reg.  */

static uint64_t elroy_config_data_read(void *opaque, hwaddr addr, unsigned len)
{
    uint32_t config_addr;
    uint64_t val;

    PCIHostState *s = opaque;
    config_addr = hp_lba_config_data_address(s->config_reg, addr);
    val = pci_data_read(s->bus, config_addr, len);
    trace_elroy_pci_config_data_read(config_addr, len, val);
    return val;
}

static void elroy_config_data_write(void *opaque, hwaddr addr,
                                   uint64_t val, unsigned len)
{
    PCIHostState *s = opaque;
    uint32_t config_addr;

    config_addr = hp_lba_config_data_address(s->config_reg, addr);
    pci_data_write(s->bus, config_addr, val, len);
    trace_elroy_pci_config_data_write(config_addr, len, val);
}

static const MemoryRegionOps elroy_config_data_ops = {
    .read = elroy_config_data_read,
    .write = elroy_config_data_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
};

static uint64_t elroy_config_addr_read(void *opaque, hwaddr addr, unsigned len)
{
    ElroyState *s = opaque;
    uint64_t value = UINT64_MAX;

    hp_sba_reg64_read(s->astro->variant->config_endianness,
                      s->config_reg_elroy, addr, len, &value);
    return value;
}

static void elroy_config_addr_write(void *opaque, hwaddr addr,
                                   uint64_t val, unsigned len)
{
    ElroyState *es = opaque;

    if (hp_sba_reg64_write(es->astro->variant->config_endianness,
                           &es->config_reg_elroy, addr, len, val)) {
        hp_lba_config_selector_sync(es->config_reg_elroy,
                                    &PCI_HOST_BRIDGE(es)->config_reg);
    }
}

static bool elroy_config_addr_access_valid(void *opaque, hwaddr addr,
                                           unsigned size, bool is_write,
                                           MemTxAttrs attrs)
{
    (void)opaque;
    (void)is_write;
    (void)attrs;
    return hp_lba_config_selector_access_valid(addr, size);
}

static const MemoryRegionOps elroy_config_addr_ops = {
    .read = elroy_config_addr_read,
    .write = elroy_config_addr_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
    .valid = {
        .min_access_size = 4,
        .max_access_size = 8,
        .unaligned = false,
        .accepts = elroy_config_addr_access_valid,
    },
    .impl = {
        .min_access_size = 4,
        .max_access_size = 8,
        .unaligned = false,
    },
};


static uint64_t astro_iommu_bypass(void *opaque, uint64_t addr)
{
    AstroState *s = opaque;

    if (addr <= UINT32_MAX) {
        return hppa_abs_to_phys_pa2_w0(s->phys_addr_bits, addr);
    }
    return addr;
}

static bool astro_iommu_pdir_read(void *opaque, uint64_t addr,
                                  uint64_t *entry)
{
    /* Astro ignores the read result. */
    *entry = ldq_le_phys(&address_space_memory, addr);
    return true;
}

/* Handle PCI-to-system address translation.  */
static IOMMUTLBEntry astro_translate_iommu(IOMMUMemoryRegion *iommu,
                                             hwaddr addr,
                                             IOMMUAccessFlags flag,
                                             int iommu_idx)
{
    AstroState *s = container_of(iommu, AstroState, iommu);
    HPSBAIOMMUParams params = {
        .ibase = s->tlb_ibase,
        .imask = s->tlb_imask,
        .pdir_base = s->tlb_pdir_base,
        .pdir_index_mask = UINT64_MAX,
        .page_shift = 12,
        .bypass = astro_iommu_bypass,
        .pdir_read = astro_iommu_pdir_read,
        .opaque = s,
    };
    HPSBAIOMMUEntry entry;

    if (!hp_sba_iommu_translate(&params, addr, &entry)) {
        return (IOMMUTLBEntry) { .perm = IOMMU_NONE };
    }

    return (IOMMUTLBEntry) {
        .target_as = &address_space_memory,
        .iova = entry.iova,
        .translated_addr = entry.translated_addr,
        .addr_mask = entry.addr_mask,
        .perm = IOMMU_RW,
    };
}

static int astro_iommu_notify_flag_changed(IOMMUMemoryRegion *iommu,
                                           IOMMUNotifierFlag old,
                                           IOMMUNotifierFlag new,
                                           Error **errp)
{
    unsigned int unsupported = (unsigned int)new &
                               ~(unsigned int)IOMMU_NOTIFIER_UNMAP;

    (void)iommu;
    (void)old;
    if (unsupported) {
        error_setg(errp,
                   "Astro IOMMU does not support notifier flags 0x%x",
                   unsupported);
        return -EINVAL;
    }
    return 0;
}

static AddressSpace *elroy_pcihost_set_iommu(PCIBus *bus, void *opaque,
                                            int devfn)
{
    ElroyState *s = opaque;
    return &s->astro->iommu_as;
}

static const PCIIOMMUOps elroy_pcihost_iommu_ops = {
    .get_address_space = elroy_pcihost_set_iommu,
};

static void astro_iommu_notify_unmap(AstroState *s, uint64_t iova,
                                     uint64_t addr_mask)
{
    const IOMMUTLBEvent event = {
        .type = IOMMU_NOTIFIER_UNMAP,
        .entry = {
            .target_as = &address_space_memory,
            .iova = iova,
            .translated_addr = 0,
            .addr_mask = addr_mask,
            .perm = IOMMU_NONE,
        },
    };

    if (s->iommu.iommu_notify_flags & IOMMU_NOTIFIER_UNMAP) {
        memory_region_notify_iommu(&s->iommu, 0, event);
    }
}

static void astro_iommu_pcom(AstroState *s)
{
    HPSBAIOMMUPurge purge;

    if (hp_sba_iommu_decode_pcom(s->tlb_pcom, 12, &purge)) {
        astro_iommu_notify_unmap(s, purge.iova, purge.size - 1);
    }
}

/* Encoding used for the reset destination in the Elroy I/O SAPIC table. */
#define SWIZZLE_HPA(a) \
        ((((a) & 0x0ff00000) >> 4) | (((a) & 0x000ff000) << 12))

static void elroy_deliver_irq(void *opaque,
                              const HPIOSAPICMessage *message)
{
    ElroyState *s = opaque;

    hp_sba_routing_deliver_interrupt(s->astro->variant, s->astro,
                                     message->address, message->data);
}

static void elroy_set_irq(void *opaque, int irq, int level)
{
    ElroyState *s = opaque;

    hp_io_sapic_set_input(&hp_io_sapic_astro_policy,
                          s->iosapic_reg, ARRAY_SIZE(s->iosapic_reg),
                          &s->ilr, irq, level, elroy_deliver_irq, s);
}

static int elroy_post_load(void *opaque, int version_id)
{
    ElroyState *s = opaque;
    PCIHostState *phb = PCI_HOST_BRIDGE(s);

    hp_lba_config_selector_sync(s->config_reg_elroy, &phb->config_reg);
    return 0;
}

static int elroy_pci_map_irq(PCIDevice *d, int irq_num)
{
    int slot = PCI_SLOT(d->devfn);

    assert(irq_num >= 0 && irq_num < ELROY_IRQS);
    return slot & (ELROY_IRQS - 1);
}

static void elroy_reset(DeviceState *dev)
{
    ElroyState *s = ELROY_PCI_HOST_BRIDGE(dev);
    PCIHostState *phb = PCI_HOST_BRIDGE(dev);

    s->config_reg_elroy = 0;
    hp_lba_config_selector_sync(s->config_reg_elroy, &phb->config_reg);

    /*
     * Make sure to disable interrupts at reboot, otherwise the Linux kernel
     * serial8250_config_port() in drivers/tty/serial/8250/8250_port.c
     * will hang during autoconfig().
     */
    hp_io_sapic_reset(&hp_io_sapic_astro_policy,
                      &s->iosapic_reg_select, s->iosapic_reg,
                      ARRAY_SIZE(s->iosapic_reg), &s->ilr,
                      SWIZZLE_HPA(CPU_HPA));
}

static void elroy_pcihost_realize(DeviceState *dev, Error **errp)
{
    ElroyState *s = ELROY_PCI_HOST_BRIDGE(dev);
    const HPSBARoutingVariant *variant = s->astro->variant;
    PCIHostState *phb = PCI_HOST_BRIDGE(dev);
    SysBusDevice *sbd = SYS_BUS_DEVICE(dev);
    Object *obj = OBJECT(s);
    g_autofree char *elroy_mmio_name = NULL;

    s->chip_ops = elroy_chip_ops;
    s->chip_ops.endianness = variant->lba_reg_endianness;
    s->config_addr_ops = elroy_config_addr_ops;
    s->config_addr_ops.endianness = variant->config_endianness;
    s->config_data_ops = elroy_config_data_ops;
    s->config_data_ops.endianness = variant->config_endianness;

    /* Elroy config access from CPU.  */
    memory_region_init_io(&s->this_mem, obj, &s->chip_ops,
                          s, dev->id, 0x2000);

    /* Elroy PCI config. */
    memory_region_init_io(&phb->conf_mem, obj,
                          &s->config_addr_ops, dev,
                          "pci-conf-idx", 8);
    memory_region_init_io(&phb->data_mem, obj,
                          &s->config_data_ops, dev,
                          "pci-conf-data", 8);
    memory_region_add_subregion(&s->this_mem, 0x40,
                                &phb->conf_mem);
    memory_region_add_subregion(&s->this_mem, 0x48,
                                &phb->data_mem);

    /* Elroy PCI bus memory.  */
    elroy_mmio_name = g_strdup_printf("%s-pci-mmio", dev->id);
    memory_region_init(&s->pci_mmio, obj, elroy_mmio_name, UINT64_MAX);
    memory_region_init_io(&s->pci_io, obj, &unassigned_io_ops, obj,
                            "pci-isa-mmio",
                            ((uint32_t)IOS_DIST_BASE_SIZE) /
                            variant->physical_rope_count);

    phb->bus = pci_register_root_bus(DEVICE(s), "pci",
                                     elroy_set_irq, elroy_pci_map_irq, s,
                                     &s->pci_mmio, &s->pci_io,
                                     PCI_DEVFN(0, 0), ELROY_IRQS, TYPE_PCI_BUS);

    sysbus_init_mmio(sbd, &s->this_mem);

    qdev_init_gpio_in(dev, elroy_set_irq, ELROY_IRQS);
}

static const VMStateDescription vmstate_elroy = {
    .name = "Elroy",
    .version_id = 1,
    .minimum_version_id = 1,
    .post_load = elroy_post_load,
    .fields = (const VMStateField[]) {
        VMSTATE_UINT64(hpa, ElroyState),
        VMSTATE_UINT32(pci_bus_num, ElroyState),
        VMSTATE_UINT64(config_address, ElroyState),
        VMSTATE_UINT64(config_reg_elroy, ElroyState),
        VMSTATE_UINT64(status_control, ElroyState),
        VMSTATE_UINT64(arb_mask, ElroyState),
        VMSTATE_UINT64_ARRAY(mmio_base, ElroyState, (0x0250 - 0x200) / 8),
        VMSTATE_UINT64(error_config, ElroyState),
        VMSTATE_UINT32(iosapic_reg_select, ElroyState),
        VMSTATE_UINT64_ARRAY(iosapic_reg, ElroyState, 0x20),
        VMSTATE_UINT32(ilr, ElroyState),
        VMSTATE_END_OF_LIST()
    }
};

static void elroy_pcihost_class_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    device_class_set_legacy_reset(dc, elroy_reset);
    dc->realize = elroy_pcihost_realize;
    dc->vmsd = &vmstate_elroy;
    dc->user_creatable = false;
}

static const TypeInfo elroy_pcihost_info = {
    .name          = TYPE_ELROY_PCI_HOST_BRIDGE,
    .parent        = TYPE_PCI_HOST_BRIDGE,
    .instance_size = sizeof(ElroyState),
    .class_init    = elroy_pcihost_class_init,
};

static void elroy_register_types(void)
{
    type_register_static(&elroy_pcihost_info);
}

type_init(elroy_register_types)


static ElroyState *elroy_init(AstroState *astro, unsigned int num,
                              const HPSBARootSpec *root)
{
    DeviceState *dev;
    ElroyState *elroy;

    dev = qdev_new(TYPE_ELROY_PCI_HOST_BRIDGE);
    dev->id = g_strdup_printf("elroy%d", num);
    elroy = ELROY_PCI_HOST_BRIDGE(dev);
    elroy->astro = astro;
    elroy->hpa = ASTRO_HPA + root->hpa_offset;
    elroy->pci_bus_num = root->bus_num;
    sysbus_realize_and_unref(SYS_BUS_DEVICE(dev), &error_fatal);

    return elroy;
}

/*
 * Astro Runway chip.
 */

static void adjust_LMMIO_mapping(AstroState *s)
{
    const HPSBARoutingVariant *variant = s->variant;
    MemoryRegion *lmmio;
    uint64_t map_addr, map_size, align_mask;
    uint32_t map_route, map_enabled;
    unsigned int i;

    lmmio = &s->lmmio;

    /* read LMMIO distributed route and calculate size */
    map_route = s->ioc_ranges[(0x370 - 0x300) / 8] >> 58;
    map_route = MIN(MAX(map_route, 20), 23);

    /* calculate size of each mapping, sum of all is 8-64 MB */
    map_size  = 1ULL << map_route;
    align_mask = ~(map_size - 1);

    /* read LMMIO_DIST_BASE for mapping address */
    map_addr  = s->ioc_ranges[(0x360 - 0x300) / 8];
    map_enabled = map_addr & 1;
    map_addr  &= MAKE_64BIT_MASK(24, 5);
    map_addr  |= MAKE_64BIT_MASK(29, 36);
    map_addr  &= align_mask;
    s->ioc_ranges[(0x360 - 0x300) / 8] = map_addr | map_enabled;

    /* make sure the lmmio region is initially turned off */
    if (lmmio->enabled) {
        memory_region_set_enabled(lmmio, false);
    }

    /* exit if range is not enabled */
    if (!map_enabled) {
        return;
    }

    if (!lmmio->name) {
        memory_region_init_io(lmmio, OBJECT(s), &unassigned_io_ops, s,
                    "LMMIO", variant->physical_rope_count * map_size);
        memory_region_add_subregion_overlap(get_system_memory(),
                    map_addr, lmmio, 1);
    }

    memory_region_set_address(lmmio, map_addr);
    memory_region_set_size(lmmio,
                           variant->physical_rope_count * map_size);
    memory_region_set_enabled(lmmio, true);

    for (i = 0; i < variant->root_count; i++) {
        MemoryRegion *alias;
        ElroyState *elroy;
        unsigned int rope;

        elroy = s->elroy[i];
        alias = &elroy->lmmio_alias;
        assert(hp_sba_routing_rope_for_root(variant, i, 0, &rope));
        if (alias->enabled) {
            memory_region_set_enabled(alias, false);
        }

        if (!alias->name) {
            memory_region_init_alias(alias, OBJECT(elroy),
                 "lmmio-alias", &elroy->pci_mmio, 0, map_size);
            memory_region_add_subregion_overlap(lmmio, rope * map_size,
                 alias, 2);
        }

        memory_region_set_address(alias, rope * map_size);
        memory_region_set_alias_offset(alias,
                (uint32_t) (map_addr + rope * map_size));
        memory_region_set_size(alias, map_size);
        memory_region_set_enabled(alias, true);
    }
}

static void adjust_LMMIO_DIRECT_mapping(AstroState *s, unsigned int reg_index)
{
    const HPSBARoutingVariant *variant = s->variant;
    MemoryRegion *lmmio_alias;
    unsigned int lmmio_index;
    unsigned int root_index;
    hwaddr map_addr;
    uint64_t map_route;
    uint32_t map_size, map_enabled;
    struct ElroyState *elroy;
    bool init_alias;

    /* each LMMIO may access from 1 MB up to 64 MB */
    const unsigned int lmmio_mask = ~(1 * MiB - 1);
    const unsigned int lmmio_max_size = 64 * MiB;

    /* pointer to LMMIO_DIRECT entry */
    lmmio_index = reg_index / 3;
    lmmio_alias = &s->lmmio_direct[lmmio_index];

    map_addr  = s->ioc_ranges[3 * lmmio_index + 0];
    map_size  = s->ioc_ranges[3 * lmmio_index + 1];
    map_route = s->ioc_ranges[3 * lmmio_index + 2];

    /* find elroy to which this address is routed */
    assert(hp_sba_routing_direct_root(variant, map_route, &root_index));
    elroy = s->elroy[root_index];

    /* make sure the lmmio region is initially turned off */
    if (lmmio_alias->enabled) {
        memory_region_set_enabled(lmmio_alias, false);
    }

    /* do sanity checks and calculate mmio size */
    map_enabled = map_addr & 1;
    map_addr &= lmmio_mask;
    map_size &= lmmio_mask;
    map_size = MIN(map_size, lmmio_max_size);
    map_addr = hp_sba_routing_extend_address(variant, s, map_addr);

    /* exit if disabled or has zero size. */
    if (!map_enabled || !map_size) {
        return;
    }

    init_alias = !lmmio_alias->name;
    if (!init_alias && lmmio_alias->alias != &elroy->pci_mmio) {
        /* Alias targets are immutable, so recreate it for a new route. */
        memory_region_del_subregion(get_system_memory(), lmmio_alias);
        object_unparent(OBJECT(lmmio_alias));
        init_alias = true;
    }

    if (init_alias) {
        char lmmio_name[32];

        snprintf(lmmio_name, sizeof(lmmio_name),
                 "LMMIO-DIRECT-%u", lmmio_index);
        memory_region_init_alias(lmmio_alias, OBJECT(elroy),
                        lmmio_name, &elroy->pci_mmio,
                        (uint32_t) map_addr, map_size);
        memory_region_add_subregion_overlap(get_system_memory(),
                        map_addr, lmmio_alias, 3);
    }

    memory_region_set_address(lmmio_alias, map_addr);
    memory_region_set_alias_offset(lmmio_alias, (uint32_t) map_addr);
    memory_region_set_size(lmmio_alias, map_size);
    memory_region_set_enabled(lmmio_alias, true);
}

static void adjust_GMMIO_mapping(AstroState *s)
{
    const HPSBARoutingVariant *variant = s->variant;
    MemoryRegion *gmmio;
    uint64_t map_addr, map_size, align_mask;
    uint32_t map_route, map_enabled;
    unsigned int i;

    gmmio = &s->gmmio;
    map_addr  = s->ioc_ranges[(0x378 - 0x300) / 8];       /* GMMIO_DIST_BASE */
    map_enabled = map_addr & 1;
    map_addr  &= MAKE_64BIT_MASK(32, 8);
    s->ioc_ranges[(0x378 - 0x300) / 8] = map_addr | map_enabled;

    map_route = s->ioc_ranges[(0x388 - 0x300) / 8] >> 58; /* GMMIO_DIST_ROUTE */
    map_route = MIN(MAX(map_route, 29), 33); /* between 4-16 GB total */
    map_size  = 1ULL << map_route;      /* size of each mapping */
    align_mask = ~(map_size - 1);

    /* make sure the lmmio region is initially turned off */
    if (gmmio->enabled) {
        memory_region_set_enabled(gmmio, false);
    }

    /* do sanity checks and calculate mmio size */
    map_addr &= align_mask;

    /* exit if disabled */
    if (!map_enabled) {
        return;
    }

    if (!gmmio->name) {
        memory_region_init_io(gmmio, OBJECT(s), &unassigned_io_ops, s,
                "GMMIO", variant->physical_rope_count * map_size);
        memory_region_add_subregion_overlap(get_system_memory(),
                map_addr, gmmio, 1);
    }

    memory_region_set_address(gmmio, map_addr);
    memory_region_set_size(gmmio,
                           variant->physical_rope_count * map_size);
    memory_region_set_enabled(gmmio, true);

    for (i = 0; i < variant->root_count; i++) {
        MemoryRegion *alias;
        ElroyState *elroy;
        unsigned int rope;

        elroy = s->elroy[i];
        alias = &elroy->gmmio_alias;
        assert(hp_sba_routing_rope_for_root(variant, i, 0, &rope));
        if (alias->enabled) {
            memory_region_set_enabled(alias, false);
        }

        if (!alias->name) {
            memory_region_init_alias(alias, OBJECT(elroy),
                 "gmmio-alias", &elroy->pci_mmio, 0, map_size);
            memory_region_add_subregion_overlap(gmmio, rope * map_size,
                 alias, 2);
        }

        memory_region_set_address(alias, rope * map_size);
        memory_region_set_alias_offset(alias, map_addr + rope * map_size);
        memory_region_set_size(alias, map_size);
        memory_region_set_enabled(alias, true);
    }
}

static void astro_rebuild_mappings(AstroState *s)
{
    unsigned int i;

    memory_region_transaction_begin();
    for (i = 0; i < s->variant->direct_range_count; i++) {
        adjust_LMMIO_DIRECT_mapping(s, i * 3);
    }
    adjust_LMMIO_mapping(s);
    adjust_GMMIO_mapping(s);
    memory_region_transaction_commit();
}

static MemTxResult astro_chip_read_with_attrs(void *opaque, hwaddr addr,
                                             uint64_t *data, unsigned size,
                                             MemTxAttrs attrs)
{
    AstroState *s = opaque;
    MemTxResult ret = MEMTX_OK;
    uint64_t val = -1;
    int index;

    switch ((addr >> 3) << 3) {
    /* R2I registers */
    case 0x0000:        /* ID */
        val = (0x01 << 3) | 0x01ULL;
        break;
    case 0x0008:        /* IOC_CTRL */
        val = s->ioc_ctrl;
        break;
    case 0x0010:        /* TOC_CLIENT_ID */
        break;
    case 0x0030:        /* HP-UX 10.20 and 11.11 reads it. No idea. */
        val = -1;
        break;
    case 0x0078:        /* NetBSD reads 0x78 ? */
        val = -1;
        break;
    case 0x0300 ... 0x03d8 - 1: /* LMMIO_DIRECT0_BASE... */
        index = (addr - 0x300) / 8;
        val = s->ioc_ranges[index];
        break;
    case 0x10200:
        val = 0;
        break;
    case 0x10220:
    case 0x10230:        /* HP-UX 11.11 reads it. No idea. */
        val = -1;
        break;
    case 0x22108:        /* IOC STATUS_CONTROL */
        val = s->ioc_status_ctrl;
        break;
    case 0x20200 ... 0x20240 - 1: /* IOC Rope0_Control ... */
        index = (addr - 0x20200) / 8;
        val = s->ioc_rope_control[index];
        break;
    case 0x20040:        /* IOC Rope config */
        val = s->ioc_rope_config;
        break;
    case 0x20050:        /* IOC Rope debug */
        val = 0;
        break;
    case 0x20108:        /* IOC STATUS_CONTROL */
        val = s->ioc_status_control;
        break;
    case 0x20310:        /* IOC_PCOM */
        val = s->tlb_pcom;
        /* QEMU MMIO writes are synchronous, so this read is the barrier. */
        break;
    case 0x20400:
        val = s->ioc_flush_control;
        break;
    default:
        if (addr >= 0x30000 && addr < 0x40000) {
            switch (addr & 0x1ff8) {
            case 0:
            case 8:
                val = 0;
                break;
            case 0x40:
            case 0x48:
                val = UINT64_MAX;
                break;
            default:
                val = 0;
                ret = MEMTX_DECODE_ERROR;
            }
        } else {
            val = 0;
            ret = MEMTX_DECODE_ERROR;
        }
    }

    /* for 32-bit accesses mask return value */
    val = mask_32bit_val(s->variant->sba_reg_endianness, addr, size, val);

    trace_astro_chip_read(addr, size, val);
    *data = val;
    return ret;
}

static MemTxResult astro_chip_write_with_attrs(void *opaque, hwaddr addr,
                                              uint64_t val, unsigned size,
                                              MemTxAttrs attrs)
{
    MemTxResult ret = MEMTX_OK;
    AstroState *s = opaque;

    trace_astro_chip_write(addr, size, val);

    switch ((addr >> 3) << 3) {
    case 0x0000:        /* ID */
        break;
    case 0x0008:        /* IOC_CTRL */
        val &= 0x0ffffff;
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->ioc_ctrl, addr, size, val);
        break;
    case 0x0010:        /* TOC_CLIENT_ID */
        break;
    case 0x0030:        /* HP-UX 10.20 and 11.11 reads it. No idea. */
        break;
    case 0x0300 ... 0x03d8 - 1: /* LMMIO_DIRECT0_BASE... */
        put_val_in_array(s->variant->sba_reg_endianness,
                         s->ioc_ranges, 0x300, addr, size, val);
        unsigned int index = (addr - 0x300) / 8;

        memory_region_transaction_begin();
        /* check if one of the 4 LMMIO_DIRECT regs, each using 3 entries. */
        if (index < s->variant->direct_range_count * 3) {
            adjust_LMMIO_DIRECT_mapping(s, index);
        }
        if (addr >= 0x360 && addr <= 0x370 + 7) {
            adjust_LMMIO_mapping(s);
        }
        if (addr >= 0x378 && addr <= 0x388 + 7) {
            adjust_GMMIO_mapping(s);
        }
        memory_region_transaction_commit();
        break;
    case 0x10200:
    case 0x10220:
    case 0x10230:        /* HP-UX 11.11 reads it. No idea. */
        break;
    case 0x20200 ... 0x20240 - 1: /* IOC Rope0_Control ... */
        put_val_in_array(s->variant->sba_reg_endianness,
                         s->ioc_rope_control, 0x20200, addr, size, val);
        break;
    case 0x20040:        /* IOC Rope config */
    case 0x22040:
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->ioc_rope_config, addr, size, val);
        break;
    case 0x20300:
    case 0x22300:
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->tlb_ibase, addr, size, val);
        break;
    case 0x20308:
    case 0x22308:
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->tlb_imask, addr, size, val);
        break;
    case 0x20310:
    case 0x22310:
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->tlb_pcom, addr, size, val);
        /* A high-lane write updates the latch but does not issue a command. */
        if (hp_sba_reg64_access_covers_low32(
                s->variant->sba_reg_endianness, addr, size)) {
            astro_iommu_pcom(s);
        }
        break;
    case 0x20318:
    case 0x22318:
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->tlb_tcnfg, addr, size, val);
        break;
    case 0x20320:
    case 0x22320:
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->tlb_pdir_base, addr, size, val);
        break;
    case 0x22000:       /* func_id */
        break;
    case 0x22008:       /* func_class */
        break;
    case 0x22050:       /* rope_debug */
        break;
    case 0x22108:        /* IOC STATUS_CONTROL */
        put_val_in_int64(s->variant->sba_reg_endianness,
                         &s->ioc_status_ctrl, addr, size, val);
        break;
    /*
     * empty placeholders for non-existent elroys, e.g.
     * func_class, pci config & data
     */
#define EMPTY_PORT(x) case x: case x+8: case x+0x40: case x+0x48:
        EMPTY_PORT(0x30000)
        EMPTY_PORT(0x32000)
        EMPTY_PORT(0x34000)
        EMPTY_PORT(0x36000)
        EMPTY_PORT(0x38000)
        EMPTY_PORT(0x3a000)
        EMPTY_PORT(0x3c000)
        EMPTY_PORT(0x3e000)
        break;
#undef EMPTY_PORT

    default:
        ret = MEMTX_DECODE_ERROR;
    }
    return ret;
}

static const MemoryRegionOps astro_chip_ops = {
    .read_with_attrs = astro_chip_read_with_attrs,
    .write_with_attrs = astro_chip_write_with_attrs,
    .endianness = DEVICE_LITTLE_ENDIAN,
    .valid = {
        .min_access_size = 4,
        .max_access_size = 8,
    },
    .impl = {
        .min_access_size = 4,
        .max_access_size = 8,
    },
};

static int astro_post_load(void *opaque, int version_id)
{
    AstroState *s = opaque;

    /* MemoryRegion topology is derived state and is not in the wire format. */
    astro_rebuild_mappings(s);
    /* Host-side translation caches are also derived state. */
    astro_iommu_notify_unmap(s, 0, UINT64_MAX);
    return 0;
}

static const VMStateDescription vmstate_astro = {
    .name = "Astro",
    .version_id = 1,
    .minimum_version_id = 1,
    .post_load = astro_post_load,
    .fields = (const VMStateField[]) {
        VMSTATE_UINT64(ioc_ctrl, AstroState),
        VMSTATE_UINT64(ioc_status_ctrl, AstroState),
        VMSTATE_UINT64_ARRAY(ioc_ranges, AstroState, (0x03d8 - 0x300) / 8),
        VMSTATE_UINT64(ioc_rope_config, AstroState),
        VMSTATE_UINT64(ioc_status_control, AstroState),
        VMSTATE_UINT64(ioc_flush_control, AstroState),
        VMSTATE_UINT64_ARRAY(ioc_rope_control, AstroState, 8),
        VMSTATE_UINT64(tlb_ibase, AstroState),
        VMSTATE_UINT64(tlb_imask, AstroState),
        VMSTATE_UINT64(tlb_pcom, AstroState),
        VMSTATE_UINT64(tlb_tcnfg, AstroState),
        VMSTATE_UINT64(tlb_pdir_base, AstroState),
        VMSTATE_END_OF_LIST()
    }
};

static void astro_reset(DeviceState *dev)
{
    AstroState *s = ASTRO_CHIP(dev);
    unsigned int i;

    s->ioc_ctrl = 0x29cf;
    s->ioc_rope_config = 0xc5f;
    s->ioc_flush_control = 0xb03;
    s->ioc_status_control = 0;
    memset(&s->ioc_rope_control, 0, sizeof(s->ioc_rope_control));

    /*
     * The SBA BASE/MASK registers control CPU -> IO routing.
     * The LBA BASE/MASK registers control IO -> System routing (in Elroy)
     */
    memset(&s->ioc_ranges, 0, sizeof(s->ioc_ranges));
    s->ioc_ranges[(0x360 - 0x300) / 8] =
        hp_sba_routing_extend_address(s->variant, s,
                                      LMMIO_DIST_BASE_ADDR) | 0x01;
    s->ioc_ranges[(0x368 - 0x300) / 8] = 0xfc000000;          /* LMMIO_DIST_MASK */
    s->ioc_ranges[(0x370 - 0x300) / 8] = 0;                   /* LMMIO_DIST_ROUTE */
    s->ioc_ranges[(0x390 - 0x300) / 8] =
        hp_sba_routing_extend_address(s->variant, s,
                                      IOS_DIST_BASE_ADDR) | 0x01;
    s->ioc_ranges[(0x398 - 0x300) / 8] = 0xffffff0000;        /* IOS_DIST_MASK    */
    s->ioc_ranges[(0x3a0 - 0x300) / 8] = 0x3400000000000000ULL; /* IOS_DIST_ROUTE */
    s->ioc_ranges[(0x3c0 - 0x300) / 8] = IOS_DIST_BASE_ADDR;  /* IOS_DIRECT_BASE  */
    s->ioc_ranges[(0x3c8 - 0x300) / 8] = 0xffffff0000;        /* IOS_DIRECT_MASK  */
    s->ioc_ranges[(0x3d0 - 0x300) / 8] = 0x0;                 /* IOS_DIRECT_ROUTE */

    s->tlb_ibase = 0;
    s->tlb_imask = 0;
    s->tlb_pcom = 0;
    s->tlb_tcnfg = 0;
    s->tlb_pdir_base = 0;

    for (i = 0; i < s->variant->root_count; i++) {
        elroy_reset(DEVICE(s->elroy[i]));
    }

    astro_rebuild_mappings(s);
    astro_iommu_notify_unmap(s, 0, UINT64_MAX);
}

static void astro_init(Object *obj)
{
    AstroState *s = ASTRO_CHIP(obj);

    s->variant = &astro_routing_variant;
}

static void astro_realize(DeviceState *obj, Error **errp)
{
    AstroState *s = ASTRO_CHIP(obj);
    const HPSBARoutingVariant *variant = s->variant;
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
    unsigned int i;

    if (!hp_sba_routing_variant_valid(variant) ||
        variant->root_count != ARRAY_SIZE(s->elroy) ||
        variant->rope_map_count != variant->root_count ||
        variant->physical_rope_count != ROPES_PER_IOC ||
        variant->direct_range_count != ARRAY_SIZE(s->lmmio_direct)) {
        error_setg(errp, "invalid Astro SBA routing variant");
        return;
    }
    for (i = 0; i < variant->root_count; i++) {
        unsigned int unused;

        if (hp_sba_routing_rope_for_root(variant, i, 1, &unused)) {
            error_setg(errp, "Astro root %u has more than one rope", i);
            return;
        }
    }

    s->chip_ops = astro_chip_ops;
    s->chip_ops.endianness = variant->sba_reg_endianness;

    memory_region_init_io(&s->this_mem, OBJECT(s), &s->chip_ops,
                          s, "astro", 0x40000);
    sysbus_init_mmio(sbd, &s->this_mem);

    /* Host memory as seen from Elroys PCI side, via the IOMMU.  */
    memory_region_init_iommu(&s->iommu, sizeof(s->iommu),
                             TYPE_ASTRO_IOMMU_MEMORY_REGION, OBJECT(s),
                             "iommu-astro", UINT64_MAX);
    address_space_init(&s->iommu_as, MEMORY_REGION(&s->iommu),
                       "bm-pci");

    /* Create Elroys (PCI host bus chips).  */
    for (i = 0; i < variant->root_count; i++) {
        const HPSBARootSpec *root = &variant->roots[i];
        ElroyState *elroy;
        hwaddr map_addr;
        uint64_t map_size;
        unsigned int rope;

        assert(hp_sba_routing_rope_for_root(variant, i, 0, &rope));
        elroy = elroy_init(s, i, root);
        s->elroy[i] = elroy;

        /*
         * NOTE: we only allow PCI devices on first Elroy for now.
         * SeaBIOS will not find devices on the other busses.
         */
        if (!root->user_attachable) {
            qbus_mark_full(&PCI_HOST_BRIDGE(elroy)->bus->qbus);
        }

        /* map elroy config addresses into Astro space */
        memory_region_add_subregion(&s->this_mem, root->hpa_offset,
                                    &elroy->this_mem);

        /* LMMIO */
        elroy->mmio_base[(0x0200 - 0x200) / 8] = 0xf0000001;
        elroy->mmio_base[(0x0208 - 0x200) / 8] = 0xf8000000;
        /* GMMIO */
        elroy->mmio_base[(0x0210 - 0x200) / 8] = 0x000000f800000001;
        elroy->mmio_base[(0x0218 - 0x200) / 8] = 0x000000ff80000000;
        /* WLMMIO */
        elroy->mmio_base[(0x0220 - 0x200) / 8] = 0xf0000001;
        elroy->mmio_base[(0x0228 - 0x200) / 8] = 0xf0000000;
        /* WGMMIO */
        elroy->mmio_base[(0x0230 - 0x200) / 8] = 0x000000f800000001;
        elroy->mmio_base[(0x0238 - 0x200) / 8] = 0x000000fc00000000;
        /* IOS_BASE */
        map_size = IOS_DIST_BASE_SIZE / variant->physical_rope_count;
        elroy->mmio_base[(0x0240 - 0x200) / 8] = rope * map_size | 0x01;
        elroy->mmio_base[(0x0248 - 0x200) / 8] = 0x0000e000;

        /* map elroys io */
        map_size = IOS_DIST_BASE_SIZE / variant->physical_rope_count;
        map_addr = hp_sba_routing_extend_address(
            variant, s, IOS_DIST_BASE_ADDR + rope * map_size);
        memory_region_add_subregion(get_system_memory(), map_addr,
                                 &elroy->pci_io);

        /* Host memory as seen from the PCI side, via the IOMMU.  */
        pci_setup_iommu(PCI_HOST_BRIDGE(elroy)->bus, &elroy_pcihost_iommu_ops,
                                 elroy);
    }
}

static const Property astro_props[] = {
    DEFINE_PROP_UINT8("phys-addr-bits", AstroState, phys_addr_bits, 32),
};

static void astro_class_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    device_class_set_legacy_reset(dc, astro_reset);
    dc->vmsd = &vmstate_astro;
    dc->realize = astro_realize;
    /*
     * astro with elroys are hard part of the newer PA2.0 machines and can not
     * be created without that hardware
     */
    dc->user_creatable = false;

    device_class_set_props(dc, astro_props);
}

static const TypeInfo astro_chip_info = {
    .name          = TYPE_ASTRO_CHIP,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_init = astro_init,
    .instance_size = sizeof(AstroState),
    .class_init    = astro_class_init,
};

static void astro_iommu_memory_region_class_init(ObjectClass *klass,
                                                 const void *data)
{
    IOMMUMemoryRegionClass *imrc = IOMMU_MEMORY_REGION_CLASS(klass);

    imrc->translate = astro_translate_iommu;
    imrc->notify_flag_changed = astro_iommu_notify_flag_changed;
}

static const TypeInfo astro_iommu_memory_region_info = {
    .parent = TYPE_IOMMU_MEMORY_REGION,
    .name = TYPE_ASTRO_IOMMU_MEMORY_REGION,
    .class_init = astro_iommu_memory_region_class_init,
};


static void astro_register_types(void)
{
    type_register_static(&astro_chip_info);
    type_register_static(&astro_iommu_memory_region_info);
}

type_init(astro_register_types)
